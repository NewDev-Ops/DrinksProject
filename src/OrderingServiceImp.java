...
public class OrderingServiceImp extends UnicastRemoteObject implements OrderingService {
    ...
    private double getPrice(Connection conn, int drinkId) throws SQLException {
        String sql = "SELECT price FROM Orders WHERE drink_id = ?";
    ...
    public String generateReport() throws RemoteException {
        ...
        try (Connection conn = DatabaseConnection.getConnection()){
            ...
            double totalSales = 0;
            while(results.next()){
                sb.append(results.getString("branch")).append(": Kes").append(results.getDouble("total_sales"));
                totalSales += results.getDouble("total_sales");
            }
            sb.append("The total amount of sales: ").append(totalSales);
        } catch (Exception e){
            e.printStackTrace();
        }
        return sb.toString();
